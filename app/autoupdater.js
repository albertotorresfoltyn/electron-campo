...
function appUpdater(updateFromMenu = false) {
  ...
  if (isDev) { // we can die without this if, trust me
    return;
  }
  ...
  log.transports.file.file = `${LogsDir}/CMFL-updates.log`;
  if (!fs.existsSync(LogsDir)) {
    fs.mkdirSync(LogsDir);
  }
  if (!fs.existsSync(log.transports.file.file)) {
    try {
      fs.writeFileSync(log.transports.file.file);
    } catch (e) {
  ...
  autoUpdater.on('error', error => {
    if (updateFromMenu) {
      ...
      dialog.showMessageBox({
        ...
        message: messageText,
        detail: (error).toString() + `\n\nThe latest version of CMFL is available at -\n our server!.\n
Current Version: ${app.getVersion()}`
      }, response => {
        if (response === 0) {
          shell.openExternal('https://codecrank.tech/');
        }
      });
      // Remove all autoUpdator listeners so that next time autoUpdator is manually called these
      // listeners don't trigger multiple times.
